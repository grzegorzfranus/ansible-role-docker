...
│   ├── logging/              # Rsyslog and logrotate configuration scenario
│   │   ├── molecule.yml     # Test configuration
...
7. **Molecule Integration Tests** — executes Molecule test matrix across Ubuntu 24.04/22.04, Debian 12/11, and Rocky Linux 9 (`ansible-molecule.yml`). Three distinct scenarios are executed: `default` (core installation, service, socket, and daemon configuration), `logging` (rsyslog, logrotate, and log owner overrides), and `uninstall` (clean removal of packages, units, and configuration while preserving system logs).
